...
declare
    ...
    pending_requesters uuid[];
    cancellation_recipients uuid[];
    event_court text;
begin
    ...
    select coalesce(array_agg(distinct sender), '{}'::uuid[])
    into pending_requesters
    ...
    into cancellation_recipients
    from unnest(
        coalesce(event_participants, '{}'::uuid[])
        || coalesce(pending_requesters, '{}'::uuid[])
    ) as recipient(user_id)
    where recipient.user_id <> current_user_id;

    if cardinality(coalesce(cancellation_recipients, '{}'::uuid[])) > 0 then
